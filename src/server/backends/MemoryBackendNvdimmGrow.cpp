...
**/
MemoryBackendNvdimmGrow::MemoryBackendNvdimmGrow(LibfabricDomain * lfDomain, const std::string & directory)
 ...
 * freed.
**/
MemoryBackendNvdimmGrow::~MemoryBackendNvdimmGrow(void)
{
	//check
	assert(chunks == 0);
...
**/
void * MemoryBackendNvdimmGrow::allocate(size_t size)
{
	//check
	assert(size > 0);
	assert(size % 4096 == 0);
...
**/
void MemoryBackendNvdimmGrow::deallocate(void * addr, size_t size)
{
	//check
	assert(addr != NULL);
	assert(size > 0);
...
**/
size_t MemoryBackendNvdimmGrow::getFileSize(void) const
{
